...
這個 workshop 的目標，是讓你親手完成一個能同時回答文件問題與資料問題的 AI PoC。你會先從可運作的範例開始，確認它真的能回答問題，再一步一步理解它為什麼能運作，最後把它改成自己的情境。

閱讀這套內容時，最有效率的方式不是一開始就鑽進底層細節，而是先把主流程跑通，再回頭把每個步驟和背後設計對起來。
...
首頁先幫你抓住主線。等你對 PoC 的執行方式有感覺之後，再回頭看這五個主軸，技術細節才比較不會變成一串抽象名詞。
...
| 路徑 | 適合誰 | 你會完成什麼 |
|------|--------|----------------|
| **管理員部署** | 你要自己把整套 Azure 與 Fabric 環境準備好 | 部署 Azure 資源、設定 Fabric，並整理出其他學員也能直接使用的環境 |
