...
如果你前面已經把主流程跑過一次，讀這頁時最值得先抓住的重點是：Foundry 並不是把所有事情都做完，它負責保存定義；真正的工具執行仍在本機 runtime 發生。
...
2. 工具合約在 Foundry 註冊，但工具本身仍在本機 runtime 執行
3. 發佈是下一階段，不是主 workshop 必做步驟
...
「Foundry 保存 agent 定義；本機 runtime 執行工具並把結果送回來。」
