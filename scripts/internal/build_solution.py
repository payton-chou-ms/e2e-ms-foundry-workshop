"""Internal build pipeline for workshop demo maintenance flows."""
...
STEPS = {
    "01": {"script": "../pipelines/data/generate_ai_data.py", "name": "產生範例資料", "time": "約 2 分鐘"},
    "02": {"script": "../pipelines/fabric/create_items.py", "name": "建立 Fabric 項目", "time": "約 30 秒"},
    "03": {"script": "../pipelines/fabric/load_lakehouse_data.py", "name": "把資料載入 Fabric", "time": "約 1 分鐘"},
    "04": {"script": "../pipelines/fabric/generate_schema_prompt.py", "name": "產生 Agent Prompt", "time": "約 5 秒"},
    "05": {"script": "../legacy/create_fabric_data_agent.py", "name": "建立 Fabric Data Agent", "time": "約 30 秒"},
    "06": {"script": "../pipelines/search/upload_documents.py", "name": "上傳到 AI Search", "time": "約 1 分鐘"},
    "06b": {"script": "../pipelines/search/create_foundry_knowledge.py", "name": "建立 Foundry IQ Knowledge Base", "time": "約 1 分鐘"},
    "07": {"script": "../pipelines/agents/create_workshop_agent.py", "name": "建立 Foundry Agent", "time": "約 10 秒"},
    "07b": {"script": "../pipelines/agents/create_foundry_iq_agent.py", "name": "建立 Foundry IQ Agent", "time": "約 10 秒"},
    "07-search": {"script": "../pipelines/agents/create_workshop_agent.py", "name": "建立 Foundry Agent（只用 Search）", "time": "約 10 秒", "args": ["--foundry-only"]},
    "08": {"script": "../pipelines/agents/test_workshop_agent.py", "name": "測試 Foundry Agent", "time": "互動式"},
    "08b": {"script": "../pipelines/agents/test_foundry_iq_agent.py", "name": "測試 Foundry IQ Agent", "time": "互動式"},
...
parser = argparse.ArgumentParser(
    description="維護者用的端到端建置流程：資料 → 知識庫 → Agent → API → App",
    formatter_class=argparse.RawDescriptionHelpFormatter,
    epilog="""
範例：
    python scripts/internal/build_solution.py                # 建立完整方案
    python scripts/internal/build_solution.py --from 04      # 從第 04 步開始
    python scripts/internal/build_solution.py --only 07      # 只執行指定步驟
    python scripts/internal/build_solution.py --skip-fabric  # 略過 Fabric 步驟
    python scripts/internal/build_solution.py --foundry-only # 不需要 Fabric，只走 Search 路徑
"""
)
...
if os.getenv("PREPARE_DEMO_INTERNAL_CALL") != "1":
    print("提示：scripts/internal/build_solution.py 是維護者入口；一般請改用 scripts/admin_prepare_docs_data_demo.py")
...
if failed:
    ...
    if args.foundry_only:
        print("\n下一步：python scripts/participant_validate_docs.py")
    elif args.foundry_iq:
        print("\n下一步：python scripts/participant_validate_foundry_iq.py")
    else:
        print("\n下一步：python scripts/participant_validate_docs_data.py")
