"""Internal prepare entry point for workshop demo orchestration."""
...
def run_preload(args: argparse.Namespace, base_env: dict[str, str]) -> bool:
    preload_cmd = [
        sys.executable,
        os.path.join(SCRIPT_DIR, "preload_scenarios.py"),
        "--scenarios",
        *args.scenarios,
    ]
    if args.skip_blob:
        ...
        if not args.skip_foundry_only_agent:
            if capabilities.get("search", False):
                success = run_step(
                    ...
                    [
                        sys.executable,
                        os.path.join(SCRIPTS_ROOT, "pipelines", "agents", "create_workshop_agent.py"),
                        "--foundry-only",
                        "--scenario",
                        scenario_key,
                    ],
        ...
        if not args.skip_foundry_iq_agent:
            if capabilities.get("foundryIq", False):
                success = run_step(
                    ...
                    [
                        sys.executable,
                        os.path.join(SCRIPTS_ROOT, "pipelines", "agents", "create_foundry_iq_agent.py"),
                        "--scenario",
                        scenario_key,
                    ],
                    scenario_env,
...
def run_build_mode(args: argparse.Namespace, base_env: dict[str, str]) -> bool:
    command = [
        sys.executable,
        os.path.join(SCRIPT_DIR, "build_solution.py"),
        "--yes",
    ]
    ...
    if args.mode == "shared":
        ...
        if args.dry_run:
            ...
            if len(args.scenarios) == 1 and args.scenarios[0] == "default":
                print("下一步：python scripts/participant_validate_docs.py")
                print("       或 python scripts/participant_validate_foundry_iq.py")
        print(f"{'='*60}")
        return 0
