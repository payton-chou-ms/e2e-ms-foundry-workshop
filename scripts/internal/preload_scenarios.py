"""Internal preload helper for shared workshop scenarios."""
...
def main():
    if os.getenv("PREPARE_DEMO_INTERNAL_CALL") != "1":
        print("提示：scripts/internal/preload_scenarios.py 是維護者入口；一般請改用 scripts/admin_prepare_shared_demo.py")
    ...
    for scenario_key in selected_keys:
        ...
        if not args.skip_blob and (not os.path.exists(custom_prepare_script) or args.skip_search):
            run_step(
                "Blob preload",
                [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "search", "upload_blob_assets.py"), "--scenario", scenario_key],
                env,
                args.dry_run,
            )

        if scenario["capabilities"].get("fabricIq") and not args.skip_fabric:
            fabric_cmd = [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "fabric", "create_items.py"), "--scenario", scenario_key]
            if args.clean_fabric:
                fabric_cmd.append("--clean")
            run_step("Fabric items", fabric_cmd, env, args.dry_run)
            run_step(
                "Fabric load",
                [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "fabric", "load_lakehouse_data.py"), "--scenario", scenario_key],
                env,
                args.dry_run,
            )
            run_step(
                "Schema prompt",
                [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "fabric", "generate_schema_prompt.py"), "--scenario", scenario_key],
                env,
                args.dry_run,
            )
        ...
        if scenario["capabilities"].get("search") and not args.skip_search:
            if os.path.exists(custom_prepare_script):
                ...
                run_step(
                    "Search upload",
                    [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "search", "upload_documents.py"), "--scenario", scenario_key],
                    env,
                    args.dry_run,
                )
        ...
        if scenario["capabilities"].get("foundryIq") and not args.skip_foundry_iq:
            run_step(
                ...
                [sys.executable, os.path.join(SCRIPTS_ROOT, "pipelines", "search", "create_foundry_knowledge.py"), "--scenario", scenario_key],
                env,
                args.dry_run,
            )
