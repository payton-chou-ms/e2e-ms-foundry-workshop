...
SCRIPT_PATH = SCRIPTS_DIR / "15b_test_multi_agent_search_only_workflow.py"
LEGACY_SCRIPT_PATH = SCRIPTS_DIR / "14b_create_multi_agent_search_only_workflow.py"


def load_workflow_module():
    sys.path.insert(0, str(SCRIPTS_DIR))
    spec = importlib.util.spec_from_file_location(
        "multi_agent_search_only_workflow_script",
        SCRIPT_PATH,
    )
    module = importlib.util.module_from_spec(spec)
    if spec.loader is None:  # pragma: no cover - defensive guard
        raise RuntimeError("Failed to load search-only script module")
    spec.loader.exec_module(module)
    return module


class MultiAgentSearchOnlyWorkflowCliTests(unittest.TestCase):
    ...
    def test_help_describes_single_script_demo_without_legacy_flags(self):
        ...
        self.assertIn("search-only", help_text)
